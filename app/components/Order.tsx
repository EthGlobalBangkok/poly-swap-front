...
function SelectedMarket ({ market }: MarketProps) {
  return (
    ...
    <div className="flex flex-row items-center space-x-4">
        <img src={market.image} alt={market.name} className="w-24 h-24 object-cover rounded-lg" />
        ...
        <div className="p-4 bg-white border border-gray-300 rounded-lg shadow-md max-w-md space-y-4">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500">
                    Select the action you want to automate if the previous trigger is activated
                </p>
            </div>
            ...
            <div className="flex justify-between items-center">
                ...
                <select
                    className="border border-gray-300 rounded-lg p-2 text-sm text-gray-700 focus:ring-blue-500 focus:border-blue-500"
                >
...
export default function Order({ market }: Props) {
    ...
    return (
        <div className="p-4">
            ...
            <Trigger selectCondition={(condition) => setIsCondYes(condition === "yes")} selectThreshold={setThreshold} />
            <Action tokens={["SOL", "BTC", "ETH"]} selectAction={setAction} selectFromAsset={setFromAsset} selectToAsset={setToAsset} selectAmount={(amount) => console.log(amount)} />
        <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors">Save automation</button>
        </div>
    );
}